...
def georeference_datastore_table(
    ...
    status_callback = make_status_callback(resource_id, job_created, {"user": "default"})
    ...
    try:
        if resource["format"].lower() == "geojson":
            geofiles.load_geojson_to_datastore(resource_id, status_callback=status_callback)

        elif "datastore_active" in resource and resource["datastore_active"]:
            postgis.prepare_and_populate_geoms(resource, status_callback=status_callback)
